...
> 数据同步与实时刷新重构设计（含前端刷新专章）见 [`docs/数据同步与实时刷新重构设计.md`](docs/数据同步与实时刷新重构设计.md)。
> 股票系统优化设计（消除一字涨停/跌停）见 [`docs/股票系统优化设计.md`](docs/股票系统优化设计.md)。
> 后端重构设计（服务端架构治理）见 [`docs/后端重构设计.md`](docs/后端重构设计.md)。
> 合同引擎与字段表达式引擎重设计（界面复杂度治理：统一 DSL/双模式编辑/图工程化/校验闭环）见 [`docs/合同与字段表达式引擎重设计.md`](docs/合同与字段表达式引擎重设计.md)。
> 重构方案总纲（六份重构蓝图整合：问题地图/依赖拓扑/决策点/里程碑路线图）见 [`docs/重构方案.md`](docs/重构方案.md)。
...
│   ├── 数据同步与实时刷新重构设计.md  # 本地存储隔离、增量同步、实时推送、前端刷新机制重构方案（S 系列 + F 系列）
│   ├── 股票系统优化设计.md            # 股票系统优化（定价公式/做市商/干预机制/参数配置）
│   ├── 后端重构设计.md                # 后端服务端架构治理（服务拆分/数据模型/同步实时/安全可观测/测试CI）
│   ├── 合同与字段表达式引擎重设计.md   # 合同引擎与字段表达式引擎界面复杂度治理（统一DSL/双模式编辑/校验闭环）
│   ├── 重构方案.md                    # 重构方案总纲（五份蓝图整合：问题地图/依赖拓扑/决策点/里程碑路线图）
